...
{
    public:
		GdbSrvControllerImpl(_In_ const std::vector<std::wstring> &coreNumberConnectionParameters) :
    ...
    //
    SimpleCharBuffer ExecuteExdiGdbSrvMonitor(_In_ unsigned core, _In_ LPCWSTR pCmdToExecute)
    {
	...
	//  false           Otherwise.
	//
    bool AttachGdbSrv(_In_ const std::wstring &connectionStr, _In_ unsigned core)
    {
    ...
    //  false               Otherwise.
    //
    bool ConnectGdbSrvCore(_In_ const std::wstring &connectionStr, _In_ unsigned core)
    {
        assert(m_pRspClient != nullptr);
    ...
    //  false           Otherwise.
    //
    bool CloseGdbSrvCore(_In_ const std::wstring &closeStr, _In_ unsigned core)
    {
        assert(m_pRspClient != nullptr);
    ...
    //  false           Otherwise.
    //
    bool ConfigureGdbSrvCommSession(_In_ bool fDisplayCommData, _In_ unsigned core)
    {
        assert(m_pRspClient != nullptr);
    ...
    //
    std::string ExecuteCommandEx(_In_ LPCSTR pCommand, _In_ bool isRspWaitNeeded, _In_ size_t stringSize)
    {
    ...
    //  The command response.
    //
    std::string ExecuteCommandOnProcessor(_In_ LPCSTR pCommand, _In_ bool isRspWaitNeeded, _In_ size_t stringSize,
    ...
    //
    std::string ExecuteCommand(_In_ LPCSTR pCommand)
    {
    ...
    //  OK
    //
    void SetRegisters(_In_ unsigned processorNumber, 
    ...
    //      +
    //
    SimpleCharBuffer ReadMemory(_In_ AddressType address, _In_ size_t maxSize, 
    ...
    //
    bool FindPcAddressFromStopReply(_In_ const std::string & cmdResponse,
    ...
    //  false       Otherwise.
    //
    inline bool IsReplyOK(_In_ const std::string & reply)
    {
        bool isOk = false;
    ...
    //
    inline RSP_Response_Packet GetRspResponse(_In_ const std::string & reply)
    {
    ...
    //  false           Otherwise.
    //
    inline bool IsReplyError(_In_ const std::string & reply)
    {
        bool isError = false;
    ...
    //
    inline bool IsStopReply(_In_ const std::string & cmdResponse)
    {
    ...
    inline void SetTargetArchitecture(_In_ TargetArchitecture targetArch) 
    {
        m_targetProcessorArch = targetArch;
    }

    inline TargetArchitecture GetTargetArchitecture() {return m_targetProcessorArch;}

    inline unsigned GetLastKnownActiveCpu() {return m_lastKnownActiveCpu;}
    inline void SetLastKnownActiveCpu(_In_ unsigned cpu) {m_lastKnownActiveCpu = cpu;}

    inline unsigned GetNumberOfRspConnections() 
    {
        assert(m_pRspClient != nullptr);
        return static_cast<unsigned>(m_pRspClient->GetNumberOfStreamConnections());
    }

    inline void DisplayLogEntry(_In_reads_bytes_(readSize) const char * pBuffer, _In_ size_t readSize)
    {
    ...
    inline int GetFirstThreadIndex() {return m_ThreadStartIndex;}
    ...
    inline bool GetThrowExceptionEnabled()
    {
    ...
    inline void SetExdiFunctions(_In_ PCWSTR pFunctionText, _In_ ExdiFunctions function)
    {
        m_exdiFunctions[std::wstring(pFunctionText)] = function;
    }

    //  Get a pointer to the target architecture array.
    const RegistersStruct * GetRegisterArrayTargetPtr(_Out_opt_ size_t * pNumberOfElements)
    {
    ...
    const RegistersStruct * FindRegisterEntry(_In_ const std::string regName)
    {
    ...
    const RegistersStruct * FindRegisterEntryAndNumberOfElements(_In_ const std::string regName,
    ...
    void FillNeonRegisterNameArrayEntry(_In_ const RegistersStruct * pRegEntry, 
